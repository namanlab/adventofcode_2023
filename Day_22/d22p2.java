...
public class d22p2 {
    ...
    public static long getTumble(int[][][] origarr, int maxX, int maxY, int maxZ, int valTM){
        int[][][] arr = new int[maxZ + 1][maxX + 1][maxY + 1];
        for (int p = 0; p <= maxZ; p++){for (int q = 0; q <= maxX; q++){for (int r = 0; r <= maxY; r++){
            arr[p][q][r] = origarr[p][q][r];
            if (arr[p][q][r] == valTM){arr[p][q][r] = 0;}
        }}}
        HashSet<Integer> effHS = new HashSet<>();
        for (int k = 1; k <= maxZ; k++){
            // Create hashmap of brick id vs list of  coordinates
            HashMap<Integer, ArrayList<ArrayList<Integer>>> hm = new HashMap<>();
            for (int i = 0; i <= maxX; i++){
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < newArr.length; i++){
            if (!newArr[i]){res += getTumble(arr, maxX, maxY, maxZ, i + 1);}
        }
