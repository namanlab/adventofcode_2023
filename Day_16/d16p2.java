...
public class d16p2 {
    ...
    public static long getOne(ArrayList<ArrayList<Character>> pats, int x, int y, int dir){
    ...
    public static void main(String[] args) {
        ...
        long res = 0;
        for (int j = 0; j < pats.get(0).size(); j++){
            long temp = getOne(pats, 0, j, 2);
            System.out.println(temp);
            res = Math.max(res, temp);
        }
        for (int j = 0; j < pats.get(pats.size() - 1).size(); j++){
            long temp = getOne(pats, pats.size() - 1, j, 0);
            System.out.println(temp);
            res = Math.max(res, temp);
        }
